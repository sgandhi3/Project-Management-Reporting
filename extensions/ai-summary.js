...
function formatDataForPrompt(data) {
  ...
  lines.push('Workstream | Planned | Executed | Passed | Failed | Not Started | In Progress | Blocked | Paused');
  for (const [ws, s] of Object.entries(data.stats)) {
    const pct = s.executed ? Math.round((s.passed / s.executed) * 100) : 0;
    lines.push(`${ws} | ${s.planned} | ${s.executed} | ${s.passed} | ${s.failed} | ${s.notStarted} | ${s.inProgress} | ${s.blocked ?? 0} | ${s.paused ?? 0} (${pct}% pass rate)`);
  }
  const c = data.consolidatedData;
  const totalPct = c.executed ? Math.round((c.passed / c.executed) * 100) : 0;
  lines.push(`TOTAL | ${c.planned} | ${c.executed} | ${c.passed} | ${c.failed} | ${c.notStarted} | ${c.inProgress} | ${c.blocked ?? 0} | ${c.paused ?? 0} (${totalPct}% pass rate)`);
  lines.push('');
